...
public class AlarmService {
    ...
    @Transactional
    public void sendCommentAlarm(String username, Long postId) {
        log.info("댓글 알람 발생 : username = {}, postId = {}", username, postId);
        // 댓글 작성자 (sender) 조회
        User sender = userRepository.findByUsername(username).orElseThrow(() -> new IllegalArgumentException("댓글 작성자 정보를 가져올 수 없습니다."));
        // 게시글 작성자(알림 받을 사람) 조회
        User receiver = getPostWriterById(postId);
        ...
        Alarm alarm = Alarm.builder()
                .sender(sender)
                .receiver(receiver)
                .content(sender.getUsername() + "님이 회원님의 게시글에 댓글을 남겼습니다.")
                .postsId(postId)
                .state(false)
